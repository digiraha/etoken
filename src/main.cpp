...
uint256 hashGenesisBlock("0x739135066bbaa3ae6d289046d90823eed2b4c7deb6bff1af5486a1bcab2de020");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eToken: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eToken Signed Message:\n";
...
{
    // eToken: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eToken: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // eToken: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x739135066bbaa3ae6d289046d90823eed2b4c7deb6bff1af5486a1bcab2de020");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "06/JAN/2014 - 30000 Meals Fed to The Homeless With Bitcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	txNew.vout[0].scriptPubKey = CScript() << 0x0 << OP_CHECKSIG;
        ...
        block.nVersion = 1;
        block.nTime    = 1389053047;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2286941;
        ...
        assert(block.hashMerkleRoot == uint256("0x53bc8d6f4223cfdc31a42d854006a4b11b16148a5ad94a2116d677230ec38d64"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // eToken: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eTokenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eTokenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eTokenMiner(CWallet *pwallet)
{
    printf("eTokenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("etoken-miner");
    ...
    try { loop {
        ...
        printf("Running eTokenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eTokenMiner, pwallet));
}
